...
function applyDefaults(): void {
  ...
  // Browser control is not in this build. Every other thing a coding agent does here is undone by
  // the user's own git history; an email sent from a signed-in profile is not, and that is the one
  // thing this product cannot offer to put right. A machine that enabled it under an older version
  // still has the file and may still have the variable, so nothing reads either any more.
